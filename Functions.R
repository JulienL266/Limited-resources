...
#A. full-sim: this function is the primary function called for generating simulation results
## (i) It outputs a results object (a list) with: 
### (a) a MC simulated dataset of size n, with factual and counterfactual data; 
### (b) computational times
### (c) a vextor of means corresponding to: 1) true factual outcomes (cluster average); 2) true counterfactual outcomes (deterministic regime); 3) true counterfactual outcomes (stochastic regime); 4) and computed counterfactual outcome (given by g-formula using known observed data parameters) (only computed if estimate = F)
### (d) a vector of probabilities corresponding to PMF for number of patients with the outcome, corresponding to the regimes in i.c.1 - i.c.3 above
## (ii) There are three secondary functions it calls:
### (a) sim function (see below)
### (b) gcomp function (see below)
### (c) table_counts
## (iii) Arguments
### (a) n: cluster size
### (b) kappa: proportion of cluster that can possibly receive treatment
### (c) p_Y_i: specifies conditional outcome means { E[Y |A=a, L=l] = q(a,l) } for a 4-level L-covariate as follows. Takes as input a vector with 8 elements ordered as follows: q(0,1); q(1,1); q(0,2); q(1,2); q(0,3); q(1,3); q(0,4); q(1,4)
### (d) p_L_i: specifies marginal covariate distribution (4 levels) (NULL if conditional=T, see below)
### (e) seed: sets seed for MC simulation (default to 1).
### (f) conditional: specifies whether or not to compute for a conditional estimand (see gcomp and sim functions)
### (g) comp_in: specifies composition for conditional estimand. defaults to NULL for marginal estimand, otherwise must be specified if conditional=T  (see gcomp and sim functions).
### (h) estimate: set to T when implementing MC gformula estimators (suppresses plug-in g-ccomputation).
### (i) B: number of simulations.


#B. gcomp: this function computes the g-formula using the functional in Theorem 1, taking as input the regime g, and the known parameters p_Y_i and p_L_i.
## (i) when conditional = T, the distribution specified by comp_in is used instead of p_L_i, which may be NULL


#C. sim: this function implements the monte-carlo simulations
## (i) It outputs a list of matrices (one for each variable in the simulation), each with B rows and n columns  
## (ii) Data are generated according to strutural equations with independent errors (thus a special case of the FFRCISTG). Strutural equations are specified by the Structural Equation Functions.
### (a.) the deterministic regime g and the stochastic regime g' are speccified by their respective structural equations
### (b.) Counterfactual outcomes under the stochastic regime are generated under the stochastic I-DTR specified by the marginal intervention distribution p_A_gplus_l, computed by the gcomp function from known parameters.
## (iii) When conditional = T, the cluster composition is fixed across simulations according to comp_in, and p_L_i is thus suppressed.


#D. table_counts: This function computes the proportions of the simulated clusters with each of the possible counts of outcomes (0 to n)
